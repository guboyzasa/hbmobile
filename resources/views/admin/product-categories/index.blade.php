...
@section('title') จัดการหมวดหมู่สินค้า | Admin - HB Mobile Services @endsection
...
@section('content')
    ...
    @component('components.breadcrumb')
        ...
        @slot('title') จัดการหมวดหมู่สินค้า @endslot
    @endcomponent
...
<script>
        ...
        $('#simple_table').ready(function () {
            simple = $('#simple_table').DataTable({
                ...
                "ajax": {
                    "url": "{{ route('admin.product-cat.show') }}",
                    "method": "POST",
                    "data": {
                        "_token": "{{ csrf_token()}}",
                    },
        ...
        $(".create_btn").click(function () {
            ...
            $('#modal_title').text('เพิ่มหมวดหมู่ใหม่');
            $('.formInput').val('');
            $('#output').attr('src','');
            $('#showImg').hide();
        ...
        function changeStatus(id){
                $.post("{{  route('admin.product-cat.change-status')  }}", data = {
                    _token: '{{ csrf_token() }}',
                    id: id,
                },
                function (res) {
        ...
        function showInfo(obj){
            ...
            $('#modal_title').text('แก้ไขชื่อหมวดหมู่');
            document.getElementById("imageFile").value = "";
            $('#imgbase64').val('');
            $('#showImg').show();
            $('#id').val(obj.id);
