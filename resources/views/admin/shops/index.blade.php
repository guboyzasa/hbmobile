...
@section('title') จัดการแบรนด์สินค้า | Admin - HB Mobile Services  @endsection
...
<script>
                ...
                "columns": [
                    {
                        ...
                        "render": function (data, type, full) {
                            ...
                            if(full.logo_img){
                                text = `<a href="#" onclick="showInfoImg('${full.logo_img}')"><img src="{{ URL::asset('${full.logo_img}') }}" alt="" class="avatar-md h-auto d-block rounded center"></a>`;
                            }
          ...
          
                    {
                        ...
                        "render": function (data, type, full) {
                            var text = ``;
                            if(data == 1){
                                text = `<input type="checkbox" onclick="changeStatus(${full.id})" id="switch${full.id}" switch="danger" checked />
                                    <label for="switch${full.id}" data-on-label="เปิด" data-off-label="ปิด"></label>`;
                            }else{
                                   text = `<input type="checkbox" onclick="changeStatus(${full.id})" id="switch${full.id}" switch="danger"  />
                                    <label for="switch${full.id}" data-on-label="เปิด" data-off-label="ปิด"></label>`;
                            }
                            return text;
                        }
        ...
        function showInfo(obj){
            ...
            $('#id').val(obj.id);
            $('#name').val(obj.name);
            $('#output').attr('src', `{{ URL::asset('${obj.logo_img}') }}`);
            $('#simpleModal').modal("show");
